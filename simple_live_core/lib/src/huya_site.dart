...
class HuyaSite implements LiveSite {
  ...
  Future<LiveSearchRoomResult> searchRooms(String keyword, {int page = 1}) async {
    var resultText = await HttpClient.instance.getJson("https://search.cdn.huya.com/", queryParameters: {
      "m": "Search", 
      "do": "getSearchContent", 
      "q": keyword, 
      "v": 4, 
    ...
    // 优先使用 response["1"] (主播数据)，因为它有准确的 room_id
    var docs = response["1"]?["docs"];
    bool useType1 = docs != null && docs.isNotEmpty;
    
    // 如果 response["1"] 不可用，回退到 response["3"] (直播间数据)
    if (!useType1) {
      docs = response["3"]?["docs"] ?? [];
    }
    ...
    for (var item in docs) {
      try {
        ...
        // 获取房间ID的逻辑
        if (useType1) {
          // response["1"] 中有 room_id 字段
          if (item["room_id"] != null && item["room_id"] != 0) {
            rId = item["room_id"].toString();
          }
        } else {
          // response["3"] 中使用 game_subChannel 或 uid
          rId = (item["game_subChannel"] ?? item["uid"] ?? "0").toString();
        }
        
        if (rId == "0") continue;

        // 封面URL处理 - 多级 Fallback 并添加处理参数
        String coverUrl = "";
        if (useType1) {
          coverUrl = item["game_avatarUrl180"]?.toString() ?? "";
        } else {
          coverUrl = item["game_screenshot"]?.toString() ?? 
        ...
        if (coverUrl.isNotEmpty && !coverUrl.contains("?")) {
          coverUrl += "?x-oss-process=style/w338_h190&";
        }

        // 标题处理
        String title = "";
        if (useType1) {
          title = item["live_intro"]?.toString() ?? "";
        } else {
          title = item["game_introduction"]?.toString() ?? 
                  item["live_intro"]?.toString() ?? "";
        }

        // 用户名
        String userName = item["game_nick"]?.toString() ?? "";

        // 在线人数
        int online = 0;
        if (useType1) {
          online = int.tryParse(item["game_activityCount"]?.toString() ?? "0") ?? 0;
        } else {
          online = int.tryParse(item["game_total_count"]?.toString() ?? 
                                item["game_activityCount"]?.toString() ?? "0") ?? 0;
        }
    ...
    int numFound = 0;
    if (useType1) {
      numFound = response["1"]?["numFound"] ?? 0;
    } else {
      numFound = response["3"]?["numFound"] ?? 0;
    }
    
    return LiveSearchRoomResult(hasMore: numFound > (page * 20), items: items);
  }
