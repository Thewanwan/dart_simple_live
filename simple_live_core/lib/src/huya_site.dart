...
class HuyaSite implements LiveSite {
  ...
  Future<LiveCategoryResult> getCategoryRooms(LiveSubCategory category, {int page = 1}) async {
    ...
    for (var item in result["data"]["datas"]) {
      String cover = item["screenshot"].toString();
      // 统一处理封面协议和缩略图
  ...
  Future<LiveRoomDetail> getRoomDetail({required String roomId}) async {
    ...
    if (cover.isEmpty) {
        // 尝试从另一个可能的地方获取
        cover = rootData["sScreenshot"]?.toString() ?? "";
    }
    if (cover.startsWith("//")) cover = "https:$cover";
    // --------------------
    ...
    var huyaBitRates = <HuyaBitRateModel>[];
    var biterates = tLiveInfo["tLiveStreamInfo"]?["vBitRateInfo"]?["value"] ?? [];
    for (var item in biterates) {
      String name = item["sDisplayName"].toString();
      if (name.contains("HDR")) continue;
      huyaBitRates.add(HuyaBitRateModel(bitRate: item["iBitRate"], name: name));
    }
  ...
  Future<LiveSearchRoomResult> searchRooms(String keyword, {int page = 1}) async {
    ...
    for (var item in docs) {
      try {
        var rId = (item["room_id"] != null && item["room_id"] != 0) 
            ? item["room_id"].toString() 
            : (item["game_subChannel"] ?? item["game_id"] ?? "0").toString();
        ...
        // 搜索结果的封面也需要添加处理参数，否则加载可能非常慢或无法显示
        if (coverUrl.isNotEmpty && !coverUrl.contains("?")) coverUrl += "?x-oss-process=style/w338_h190&";
